...
void GameLoop::AddUI(shared_ptr<UI> u) {
	CheckReference();
	m_instance->m_uis.Add(u);
}
void GameLoop::DelUI(UI& u) {
	CheckReference();
	m_instance->m_uis.Del(u);
}
shared_ptr<UI> GameLoop::GetUIByName(string name) {
	CheckReference();
	for (int i = 0; i < m_instance->m_scenes.Count(); i++) {
		auto t = m_instance->m_uis[i];
		if (t->GetName() == name) {
			return t;
		}
	}
	return shared_ptr<UI>();
}
void GameLoop::Clear() {
	CheckReference();
	...
	m_instance->m_uis.Delete();
}
void GameLoop::ClearAllButScenes() {
	CheckReference();
	Log("CLEAR ALL BUT SCENES");
	m_instance->m_chars.Clear();
...
Vector2 GameLoop::GetMousePosition() {
	CheckReference();
...
void GameLoop::Graphics(float delta) {
	...
	m_uis.Iterate([this](shared_ptr<UI> i) -> void {
		i->Draw();
	});
	FlushBatchDraw();
}
...
void GameLoop::Input() {
	while (peekmessage(&m_msg)) {
		...
		if (m_msg.message == WM_MOUSEMOVE || m_msg.message == WM_MOUSEWHEEL ||m_msg.message ==  WM_LBUTTONDOWN || m_msg.message == WM_LBUTTONUP || 
			m_msg.message == WM_LBUTTONDBLCLK || m_msg.message == WM_MBUTTONDOWN || m_msg.message == WM_MBUTTONUP || m_msg.message == WM_MBUTTONDBLCLK || 
			m_msg.message == WM_RBUTTONDOWN || m_msg.message == WM_RBUTTONUP || m_msg.message == WM_RBUTTONDBLCLK) {
			m_mousePosition.x = m_msg.x;
			m_mousePosition.y = m_msg.y;
		}
	}
}
void GameLoop::Update() {
	...
	while (m_isRunning) {
		...
		m_uis.Iterate([this, delta](shared_ptr<UI> i) -> void {
			i->Update(delta);
		});
		m_uis.ClearBin([](shared_ptr<UI> i) -> void {
			i->Delete();
		});
		m_uis.AddToContainer([](shared_ptr<UI> i) -> void {
			i->Awake();
		});
		Physics();
	}
