...
void Character::InitAnimation(float animationSpeed, string LImgPath[], int LN, string RImgPath[], int RN, string shadowPath, float sx, float sy) {
	m_animation->SetAnimationSpeed(animationSpeed);
	m_animation->SetLImgs(LImgPath, LN);
	m_animation->SetRImgs(RImgPath, RN);
	m_animation->SetShadow(shadowPath);
